...
NSString * const DataBaseName  = @"FindPlacesData.sqlite"; // Paas Your DataBase Name Over here
        ...
        if((sqlite3_prepare_v2(databaseObj,[query UTF8String],-1, &statement, NULL)) == SQLITE_OK)
        {
            while(sqlite3_step(statement) == SQLITE_ROW)
            {
                ...
                for (int i=0; i < count; i++)
                {
                    ...
                    NSString *strData = [NSString stringWithFormat:@"%s",sqlite3_column_text(statement,i)];
                    
                    strData = [strData stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
                    
                    NSString *columnName = [NSString stringWithCString:name encoding:NSUTF8StringEncoding];
                    
                    
                    [currentRow setObject:strData forKey:columnName];
                }
	...
	if(sqlite3_open([path UTF8String],&databaseObj) == SQLITE_OK ) {
		...
		if((sqlite3_prepare_v2(databaseObj, [query UTF8String], -1, &statement, NULL)) == SQLITE_OK) {
			...
			while(sqlite3_step(statement) == SQLITE_ROW) {
				...
				[workoutArray addObject:currentRow ];
			}
		}
		sqlite3_finalize(statement);
	}
